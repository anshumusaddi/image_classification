...
def save_json(results: Dict, filename: str, float_scores: bool = False) -> None:
    ...
    if float_scores:
        for _file, dup_list in results.items():
            if dup_list:
                type_casted_dup_list = []
                for dup in dup_list:
                    type_casted_dup_list.append((dup[0], float(dup[1])))

                results[_file] = type_casted_dup_list

    with open(filename, 'w') as f:
        json.dump(results, f, indent=2, sort_keys=True)


def parallelize(function: Callable, data: List, verbose: bool) -> List:
