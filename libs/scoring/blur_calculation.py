...
def calculate_image_blur_score(crisp, sharpness):
    return crisp + sharpness
...
def calculate_blur_score(clusters_data):
    ...
    blur_score_list = [calculate_image_blur_score(i, j) for i, j in zip(norm_crisp_list, norm_sharpness_list)]
    ...
    for cluster in clusters_data:
        for value in cluster.values():
            ...
            value["blur_score"] = calculate_image_blur_score(value["normalized_crisp"], value["normalized_sharpness"])
