...
def calculate_face_score(clusters_data):
    ...
    for cluster in clusters_data:
        for value in cluster.values():
            ...
            for face in value.get("faces", []):
                ...
                tebb = get_TEBB_score(face["normalized_dct_blur"], get_blink_flag(face), emo_score)
                face["tebb"] = tebb
    ...
    for cluster in clusters_data:
        for value in cluster.values():
            value["normalized_face_score"] = min_max_normalize(value["image_face_score"], face_score_min,
                                                               face_score_max)
...
def get_blink_flag(face):
    if face["face_feature"]["dominant_race"] == "asian":
        blink_thresh = 1
    else:
        blink_thresh = 2
    ...
    if yaw > 30 or yaw < -30 or pitch > 30 or pitch < -30 or face["face_feature"]["emotion"]["sad"] > 90 or \
            face["face_feature"]["emotion"]["happy"] > 90:
        blink_flag = 1
    else:
